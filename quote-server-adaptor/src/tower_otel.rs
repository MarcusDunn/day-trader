...
impl <T> OtelLayer<T> where T: Tracer {
...
impl<S, T> Layer<S> for OtelLayer<T> where T: Clone {
    type Service = OtelService<S, T>;

    fn layer(&self, inner: S) -> Self::Service {
        OtelService { inner, tracer: self.tracer.clone() }
    }
}
        ...
        T::Span: Send + 'static,
{
    ...
    fn call(&mut self, mut req: Request<B>) -> Self::Future {
        let context = opentelemetry::global::get_text_map_propagator(|prop| prop.extract(&HeaderMap(req.headers_mut())));
        let uri = req.uri();

        let builder = self.tracer.span_builder(uri.path().to_string());
        let mut span = self.tracer.build_with_context(builder, &context);

        let fut = self.inner.call(req).map(move |res| {
            match res {
                Ok(mut ok) => {
                    opentelemetry::global::get_text_map_propagator(|prop| prop.inject(&mut HeaderMap(ok.headers_mut())));
                    span.end();
                    Ok(ok)
                }
                Err(e) => {
