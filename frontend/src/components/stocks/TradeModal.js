	...
	const CommitAction = async () => {
		...
		try {
			...
			if (response_parsed.success == true) {
				...
				setError(`Unsuccessful Commit ${action.charAt(0).toUpperCase() + action.slice(1)} Attempt`);
			}
		} catch (error) {
			console.log(error);
			setError(`Unsuccessful Commit ${action.charAt(0).toUpperCase() + action.slice(1)} Attempt`);
	...
	const CancelAction = async () => {
		...
		try {
			...
			if (response_parsed.success == true) {
				...
				setError(`Unsuccessful Cancel ${action.charAt(0).toUpperCase() + action.slice(1)} Attempt`);
			}
		} catch (error) {
			console.log(error);
			setError(`Unsuccessful Cancel ${action.charAt(0).toUpperCase() + action.slice(1)} Attempt`);
			...
			{readyToCommit ? (
				<div>
					<Typography variant="h4" gutterBottom>
						Commit {stock.name} {action.charAt(0).toUpperCase() + action.slice(1)}
					</Typography>
					<Typography
						variant="p"
						display={"block"}
					...
					<Button className="mr-4" variant="outlined" color="secondary" onClick={CommitAction}>
						Commit {action.charAt(0).toUpperCase() + action.slice(1)}
						</Button>
						<Button className="mr-4" variant="outlined" color="primary" onClick={CancelAction}>
						Cancel
						</Button>
					</div>
				</div>
				...
				<div>
						...
						<FormControl >
								<div className="mb-6 w-40">
									...
									<Select fullWidth labelId="action-select-label" value={action} onChange={(e) => setAction(e.target.value)}>
